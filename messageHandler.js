...
import { getBatchEmbeddings, getEmbedding } from './improvedEmbeddings.js';
import { getCachedUser, invalidateUserCache, warmupUserCache, cachedVectorSearch } from './databaseCache.js';
...
const SUMMARY_THRESHOLD = 3;             // Number of messages before summarization
const INACTIVITY_THRESHOLD = 8 * 60 * 60 * 1000; // 8 hours inactivity threshold

// ========== STATE MANAGEMENT ==========

// Track user activity times (with guild context)
const userLastActive = new Map();
// Store the last summary timestamp per user (with guild context)
const lastSummaryTimestamps = new Map();
// Track new messages since last extraction (with guild context)
const userMessageCounters = new Map();

// ========== UTILITY FUNCTIONS ==========

/**
 * Normalizes text for comparison to detect similar memories
 */
...
async function isDuplicateMemory(userId, memoryText, guildId) {
  try {
    ...
    // Get existing memories in this category (with guild filter)
...
async function summarizeAndExtract(userId, conversation, guildId) {
  try {
    ...
    // Use the enhanced two-stage memory extraction - passing guildId AND botName
...
/**
 * Handles non-slash command messages for the bot
 * @param {Message} message - The Discord message
 */
